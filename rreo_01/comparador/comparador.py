...
def comparar_intervalo(sheet1, sheet2, inicio='A1', fim='K219'):
    discrepancias = []
    for celula in sheet1.range(f'{inicio}:{fim}'):
     
        row_index = celula.row
        column_index = celula.column

        
        if (row_index in range(21, 99) or row_index in range(139, 202)) and column_index in [5, 7]:
            continue  
...
def main():
    st.title('Comparador RREO Anexo 01')
    ...
    if st.button('Comparar Arquivos'):
        if arquivo_xls_1 and arquivo_xls_2:
            ...
            try:
                ...
                sheet1 = wb1.sheets['RREO-Anexo 01']
                sheet2 = wb2.sheets['RREO-Anexo 01']
