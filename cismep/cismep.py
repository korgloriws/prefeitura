...
def main():
    ...
    if uploaded_file:
        ...
        if st.button('Filtrar e desmembrar coluna'):
            ...
            filtered_data['NATUREZA DESPESA / SUBELEMENTO COM 6 DIGITOS'] = (
                filtered_data[column_name].str.replace('.', '').str.slice(0, 6)
            )
            filtered_data['NATUREZA DESPESA / SUBELEMENTO COM 2 DIGITOS'] = (
                filtered_data[column_name].str.replace('.', '').str.slice(-2)
            )
            ...
            for col in ['Mês Referência', 'COD. FUNÇÃO']:
                if col in filtered_data.columns:
                    filtered_data[col] = filtered_data[col].apply(lambda x: x.zfill(2) if pd.notnull(x) else x)

     
            filtered_data['CO'] = ''


            filtered_data['Registro'] = 30
            ...
            final_cols_order = [
                ...
                'Valor de empenhos anulados no mês',
