...
def padronizar_dataframe(df):
    ...
    df['cod_contabil'] = df['cod_contabil'].astype(str).str.strip()

    df.fillna(0, inplace=True)
    

    numeric_columns = ['debito_anterior', 'credito_anterior', 'debito_mes', 'credito_mes', 'debito_atual', 'credito_atual']
    for col in numeric_columns:
        df[col] = df[col].astype(str).str.replace('.', '').str.replace(',', '.').astype(float).round(2)
...
def main():
    st.title(' balancete Camara VS balancete Sicof')
