    ...
    if ded_data is not None:
        for row in ded_data.itertuples():
            ...
            if row.fonte == '52500701':
                ...
                soma_valores_G65 += row.pag_ate_mes

    sheet.range('G61').value = soma_valores_G61
    sheet.range('G65').value = soma_valores_G65

    
    if rpnp_data is not None:
        for row in rpnp_data.itertuples():
            if row.fonte == '1500701':
                sheet.range('G67').value = row.valor_anu_ant  
                sheet.range('G75').value = row.valor_anu_mes 
            if row.fonte == '31500701':
                sheet.range('G69').value = row.valor_anu_ant  
                sheet.range('G76').value = row.valor_anu_mes  
            if row.fonte == '51500701':
                sheet.range('G70').value = row.valor_anu_ant  
                sheet.range('G77').value = row.valor_anu_mes  
            if row.fonte == '2500701':
                sheet.range('G71').value = row.valor_anu_ant  
                sheet.range('G78').value = row.valor_anu_mes  
            if row.fonte == '32500701':
                sheet.range('G72').value = row.valor_anu_ant  
                sheet.range('G79').value = row.valor_anu_mes  
            if row.fonte == '52500701':
                sheet.range('G73').value = row.valor_anu_ant 
                sheet.range('G80').value = row.valor_anu_mes  
...
def main():
    ...
    if st.button("Processar Dados"):
        if uploaded_qgr and uploaded_ded and uploaded_rpnp and uploaded_mde:
            ...
            fill_MDE(temp_file_path, qgr_data, ded_data, rpnp_data)
