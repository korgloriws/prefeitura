...
def main():
    ...
    posicao_file = st.file_uploader("Selecione o arquivo CSV para 'Posição de Bancos'", type=['csv'])
    ctb_file = st.file_uploader("Selecione o arquivo CSV para 'CTB'", type=['csv'])

    if posicao_file is not None and ctb_file is not None:
        custom_filename = st.text_input("Nome do arquivo Excel de saída:", "Diferencas_Saldos.xlsx")

        posicao_bancos = pd.read_csv(posicao_file, dtype=str, encoding='ISO-8859-1', delimiter=';', on_bad_lines='skip')
        ctb = pd.read_csv(ctb_file, dtype=str, encoding='ISO-8859-1', delimiter=';', on_bad_lines='skip')
        ...
        for idx, row_posicao in posicao_bancos.iterrows():
            ...
            if cod_reduz in ctb_grouped['cod_reduz_banco'].values:
                ...
                if is_significant_difference(saldo_inicial_posicao, saldo_inicial_ctb) or is_significant_difference(saldo_final_posicao, saldo_final_ctb):
                    differences.append({
        ...
        if st.button('Gerar Excel'):
            with pd.ExcelWriter(custom_filename) as writer:
            ...
            st.success(f"Arquivo '{custom_filename}' salvo com sucesso.")
